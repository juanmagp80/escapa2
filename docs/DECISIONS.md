...
## ADR-023: Registro de dispositivo en Android con token estable por instalación

- **Estado:** Aceptada.
- **Contexto:** El backend ya expone `POST /devices` (ADR-022). La app debe registrar el dispositivo para poder recibir notificaciones del radar, pero Firebase Messaging aún no está configurado (sin `google-services.json` ni proyecto). AGENTS.md 17 pide notificaciones con deep links.
- **Decisión:** El dispositivo envía a `POST /devices` un token estable generado por instalación (`android-<uuid>` persistido en SharedPreferences) mediante `DeviceTokenProvider`. `DeviceRepository` sigue el patrón remoto+fake+fallback del resto de repositorios. `DeviceRegistrar` es el único punto que decide registrar/borrar el token según la preferencia del usuario (persistida también en SharedPreferences) y expone el estado como `StateFlow`; se inicializa en `Escapa2Application.onCreate`. En Perfil se añade un interruptor "Notificaciones push" (`NotificationSettingsViewModel`). Cuando se configure FCM, basta cambiar `DeviceTokenProvider` para devolver el token real de Firebase Messaging sin tocar el resto.
- **Consecuencias:** El pipeline de notificaciones queda completo y testeable sin Firebase: registro → radar → envío simulado → log. La app no pide permisos ni envía tokens reales hasta que se active FCM, respetando la privacidad (AGENTS.md 13.3).

## ADR-024: Firebase Cloud Messaging real con token de FCM

- **Estado:** Aceptada.
- **Contexto:** El usuario creó el proyecto Firebase `escapa2-e0196`, aportó `google-services.json` y la cuenta de servicio del Admin SDK. AGENTS.md 17 exige FCM para las notificaciones del radar.
- **Decisión:** En Android se añaden el plugin `com.google.gms.google-services` (4.4.2), `firebase-bom` (33.9.0) y `firebase-messaging`. `DeviceTokenProvider` pasa a estar implementado por `FcmDeviceTokenProvider`, que expone `token` (el token real de FCM) y `tokenUpdates` (Flow) para capturar refrescos; `DeviceRegistrar` escucha el flujo y registra cada token nuevo mientras esté activado, dando de baja el anterior. `Escapa2MessagingService` (`@AndroidEntryPoint`) recibe `onNewToken` y lo reenvía al proveedor. Se añade `POST_NOTIFICATIONS`. En backend, `firebase-admin` entra en `requirements.txt`, `backend/.env` activa `FIREBASE_ENABLED=true` con `FIREBASE_PROJECT_ID` y `FIREBASE_CREDENTIALS_FILE`, y la cuenta de servicio queda ignorada por Git. El `FirebaseNotificationSender` ya estaba implementado (ADR-022).
- **Consecuencias:** Las notificaciones del radar se envían por FCM real cuando hay tokens registrados. La cuenta de servicio es un secreto (solo en `backend/.env` y el servidor). El registro con token local anterior queda sustituido por el token real.

## ADR-025: Deep links y notificaciones en primer plano

- **Estado:** Aceptada.
- **Contexto:** AGENTS.md 17 exige que cada notificación incluya un deep link a la pantalla correspondiente. FCM solo muestra notificaciones del sistema cuando la app está en segundo plano; en primer plano hay que renderizarlas manualmente.
- **Decisión:** El backend incluye `deep_link: escapa2://radar` en el payload FCM de las alertas del radar (`NotificationService.notify_watch_run`). En Android, `MainActivity` (launchMode `singleTask`) expone un `SharedFlow<Intent>` con el intent inicial y los de `onNewIntent`; `Escapa2NavHost` llama a `navController.handleDeepLink` solo para intents con el esquema `escapa2://` (constante `DeepLinks`), y el destino Radar declara `navDeepLink { uriPattern = DeepLinks.RADAR }` con el intent-filter `escapa2://radar` en el manifest. `Escapa2MessagingService.onMessageReceived` construye la notificación (canal `radar_alerts`, `PendingIntent` a la URI del deep link) y la muestra también en primer plano. En Perfil, al activar el interruptor se solicita el permiso runtime `POST_NOTIFICATIONS` en Android 13+.
- **Consecuencias:** Tocar una alerta del radar abre la pantalla Radar desde cualquier estado de la app. El permiso se pide solo al activar las notificaciones, alineado con AGENTS.md 13.3. Los futuros tipos (`NEW_OPPORTUNITY`, `DAILY_REPORT`) añadirán deep links propios siguiendo el mismo mecanismo.

## ADR-026: Modelo de vehículo y dominio de rutas/gasolineras para el coche

- **Estado:** Aceptada.
- **Contexto:** Fase 5 (Coche) necesita estimar costes de trayecto por carretera: combustible, peajes, aparcamiento y desgaste, además de recomendar gasolineras por ahorro neto. AGENTS.md 8.1 define `VehicleProfile`; la regla de ahorro neto de gasolinera (AGENTS.md 9.4) ya existe en `domain/fuel_stations.py`.
- **Decisión:** Se añade `VehicleProfile` (en `domain/vehicles.py`) como un único vehículo por perfil, con `travel_profile_id` (siguiendo la convención de `AirportPreference`, ya que `Couple` aún no existe) en vez de `couple_id`. Se crea la tabla `vehicle_profiles` (migración `0006`), el ORM, el repositorio SQL y los endpoints `GET/PUT /profile/vehicle`; el mock sirve un vehículo de desarrollo. Además se normalizan `RouteOffer`, `RouteRequest`, `FuelStation` y `FuelStationsRequest` en `domain/offers.py` con protocolos `RouteProvider`/`FuelPriceProvider` y mocks deterministas (`MockRouteProvider`, `MockFuelPriceProvider`). Las funciones puras `estimate_fuel_cost`, `estimate_vehicle_wear`, `route_cost_components` y `recommend_fuel_stations` operan sobre estos modelos reutilizando el cálculo de ahorro neto. Los proveedores reales (Google Routes y servicio oficial de gasolineras) se añadirán en pasos siguientes tras este MVP.
- **Consecuencias:** El backend expone el vehículo de la pareja y el dominio de rutas/gasolineras está listo para integrarse en el ensamblaje de oportunidades de coche. No se exponen secretos de Google Maps todavía (no se añaden a `render.yaml`, que se completarán en el paso de integración real).

## ADR-026: Perfil de vehículo como parte del perfil de la pareja

- **Estado:** Aceptada.
- **Contexto:** AGENTS.md 8.1 define `VehicleProfile` (con `couple_id`) para estimar costes de coche: combustible, desvíos a gasolineras (AGENTS.md 9.4) y desgaste. Hasta que exista la entidad `Couple`, el sistema usa una única fila en `travel_profiles` como proxy de la pareja (mismo patrón que `AirportPreference`).
- **Decisión:** Se crea `VehicleProfile` en `domain/vehicles.py` con `travel_profile_id` en lugar de `couple_id`, para mantener la consistencia con `AirportPreference` y la persistencia actual. Un solo vehículo por defecto en el MVP (`GET/PUT /profile/vehicle`), con `FuelType` (`DIESEL`, `GASOLINE`, `HYBRID`, `ELECTRIC`) y validación: consumo obligatorio salvo eléctrico. Se persiste en una tabla nueva (`vehicle_profiles`, migración `0006`) con mock equivalente y métodos `get_vehicle`/`save_vehicle` en `ProfileProvider`.
- **Consecuencias:** El cálculo de rutas y gasolineras (Fase 5) podrá usar `VehicleProfile` como fuente de consumo, capacidad y coste por km. Cuando existan parejas reales, `travel_profile_id` se migrará a `couple_id`.
